...
export const AIConsultPanel: React.FC<AIConsultPanelProps> = ({ 
  ...
  if (!acceptedTerms) {
      return (
        <section className="col-span-12 lg:col-span-4 h-full bg-white/90 backdrop-blur-xl rounded-3xl lg:rounded-[2.5rem] shadow-xl border border-indigo-100 flex flex-col items-center justify-center p-8 relative z-20 animate-fade-enter ring-4 ring-indigo-50 min-h-0 text-center">
            <button onClick={() => setShowExitConfirm(true)} className="absolute top-4 right-4 text-slate-400 hover:text-slate-600 z-50">
            ...
            {showExitConfirm && (
                <div className="absolute inset-0 bg-white/95 backdrop-blur-sm z-[60] flex items-center justify-center p-4 rounded-[2.5rem]">
                    <div className="bg-white border border-slate-100 shadow-xl p-6 rounded-2xl text-center w-full max-w-xs">
  ...
  return (
    <section className="col-span-12 lg:col-span-4 h-full bg-white/90 backdrop-blur-xl rounded-3xl lg:rounded-[2.5rem] shadow-xl border border-indigo-100 flex flex-col overflow-hidden relative z-20 animate-fade-enter ring-4 ring-indigo-50 min-h-0">
      ...
      {/* Header */}
      <div className="px-5 py-4 bg-gradient-to-r from-indigo-600 to-violet-600 shrink-0 flex flex-col gap-3 shadow-md relative z-20">
        <button 
            onClick={() => setShowExitConfirm(true)} 
            className="absolute top-4 right-4 text-indigo-100 hover:text-white transition p-1 bg-white/10 rounded-lg z-30"
            title={t.common.exit}
        >
      ...
      {/* Messages Area */}
      <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50 no-scrollbar min-h-0 relative z-10">
      ...
      {/* EXIT CONFIRMATION OVERLAY - MOVED HERE */}
      {showExitConfirm && (
            <div className="absolute inset-0 bg-white/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-enter">
                <div className="bg-white border border-slate-100 shadow-2xl p-6 rounded-2xl text-center max-w-xs w-full relative z-50">
