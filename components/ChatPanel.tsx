...
export const ChatPanel: React.FC<ChatPanelProps> = ({ 
  ...
  // Use Memo for Location Data - Direct from restricted ubigeo.ts
            ...
            <div className="relative" ref={menuRef}>
                ...
                {isMenuOpen && (
                    <div className="absolute right-0 top-full mt-2 w-52 bg-white rounded-xl shadow-xl border border-slate-100 py-2 animate-fade-enter origin-top-right overflow-hidden z-50">
                        <button 
                            onClick={() => { setIsMenuOpen(false); onShowData(); }}
                            className="w-full text-left px-4 py-3 text-xs font-bold text-violet-700 bg-violet-50 hover:bg-violet-100 transition flex items-center gap-2 mb-1"
                        >
                ...
                <div className="flex flex-wrap gap-2 mt-2 max-w-[95%]">
                    {INSURANCES.map(ins => (
                        <button 
                            ...
                            className="px-4 py-2 rounded-full border text-xs font-bold transition shadow-sm border-slate-200 bg-white text-slate-600 hover:border-blue-500 hover:text-blue-600 hover:bg-blue-50"
                        >
                            {ins}
                        </button>
                    ))}
